...
- [ ] **Dependency Graph & Execution Order (Rank System)**
    - [ ] Implement `Rank` property for connectivity validation.
        - Constants/Inputs: Rank 0.
        - Derived Objects: Rank = Max(Input Ranks) + 1.
    - [ ] Implement `Topological Sort` at Edit-time.
    - [ ] Execution: Iterate sorted list `executionOrder`.
    - [ ] Cycle Detection: Reject connections that violate `Rank(A) < Rank(B)` or detect cycles during sort.
