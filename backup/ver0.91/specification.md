...
## 18. 依存関係グラフと実行順序 (Rank System)

実行時のパフォーマンスと整合性を保証するため、編集時（Edit-Time）に依存関係を解決する。

### 18.1 Rank System (階層化)
* 全てのオブジェクトは `rank` (integer) を持つ。
* **定数・外部入力**: Rank 0
* **派生オブジェクト**: Rank = `Max(Input Ranks) + 1`

### 18.2 接続検証 (Cycle Prevention)
* オブジェクト A を B の入力として接続する際、`Rank(A) < Rank(B)` であるかを検証する。
* 違反する場合、B（およびその子孫）のランク再計算を試みる。
* 再計算により循環が検知された場合、接続を拒否する。

### 18.3 実行リスト
* 編集操作ごとに `Topological Sort` を行い、フラットな `executionOrder` 配列を作成する。
* 実行時（`p5.draw`）は、この配列を先頭から順に評価するだけでよい。
